...
describe('Create Car Specification', () => {
  ...
  it('should be able to add a new specification to the car', async () => {
    ...
    const createCar = await createCarSpecificationUseCase.execute({
      car_id: car.id,
      specifications_id,
    })

    expect(createCar).toHaveProperty('specifications')
    expect(createCar.specifications).toHaveLength(1)
  })
})
